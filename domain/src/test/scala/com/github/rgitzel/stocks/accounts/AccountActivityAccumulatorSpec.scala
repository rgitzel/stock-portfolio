...
class AccountActivityAccumulatorSpec extends AnyFlatSpecLike {
  ...
  "just one stock and two transactions" should s"add up correctly" in {
    val expected = AccountBalance(0, -403.0 + 438.0)
    AccountActivityAccumulator.accumulatedShareCountAndCash(journalWithOneStockOnlyTwoTransactions.activities) should be (expected)
  }
//
//  List(
//    ("after selling", TradingDay(11, 1, 2017), AccountHoldings(0.0, Map[Stock,Int]()))
//  )
//    .foreach{ case (reason, day, expectedHoldings) =>
//      it should s"be correct ${reason}" in {
//        val expected = Account(name, Map(currency -> AccountHoldings(35.0, Map())))
