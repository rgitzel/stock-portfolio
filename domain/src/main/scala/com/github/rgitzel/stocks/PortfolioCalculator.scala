...
import com.github.rgitzel.stocks.accounts.{Account, AccountCalculator, AccountJournal, AccountValuation}
import com.github.rgitzel.stocks.models._
import com.github.rgitzel.stocks.money.{Currency, MonetaryValue, MoneyConverter}
import com.github.rgitzel.stocks.repositories.{ForexRepository, IntegrityChecker, PricesRepository}
...
class PortfolioCalculator(
  ...
  private def valuationThisWeek(
    ...
    IntegrityChecker.checkForMissingPrices(accountsForThisWeek, closingPrices) match {
      case Nil =>
        val accountValuations = accountsForThisWeek.map(AccountCalculator.valuate(_, closingPrices))
  ...
  def valuate(weeks: List[TradingWeek], accountJournals: List[AccountJournal])
             (implicit ec: ExecutionContext): Future[List[(TradingWeek, PortfolioValuation)]] =
    Future.sequence(
      weeks.map { week =>
