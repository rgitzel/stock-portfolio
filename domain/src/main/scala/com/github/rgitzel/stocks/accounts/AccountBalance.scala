...
final case class AccountBalance(numberOfShares: Int, cash: Double) {
  ...
  def plusCash(value: Double): AccountBalance = this.copy(cash =
    CashUtils.roundedToCents(this.cash + CashUtils.roundedToCents(value))
  )
  def minusCash(value: Double): AccountBalance = this.plusCash(-value)

  def minusShares(amount: Int): AccountBalance =
    this.copy(numberOfShares = this.numberOfShares - amount)
