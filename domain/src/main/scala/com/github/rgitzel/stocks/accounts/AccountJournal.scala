 ...
 */
final case class AccountJournal(
    name: AccountName,
    ...
    Account(
      ...
      activities
        ...
        .map { case (currency, activities) =>
          val countsAndCashForStocks = activities
            .groupBy(_.stock)
            .view
            .map { case (stock, activitiesForStock) =>
              val x = AccountActivityAccumulator
                .accumulatedShareCountAndCash(activitiesForStock)
//              println(s"${currency} ${stock}\n  ${activitiesForStock.mkString("\n  ")}\n  ${x}")
              (stock, x)
            }
            .toMap
          val cash = CashUtils.roundedToCents(
            countsAndCashForStocks.values.map(_.cash).sum
          )
          val stocks = countsAndCashForStocks
            .mapValues(_.numberOfShares)
            .toMap
            .filter(_._2 > 0)
