...
trait PortfolioValueRepository {
  def updateAccountStockRecords(records: List[AccountStockValuationRecord])(implicit ec: ExecutionContext): Future[Int]
  def updatePortfolioRecords(records: List[PortfolioValuationRecord])(implicit ec: ExecutionContext): Future[Int]
  def updateAccountRecords(records: List[AccountValuationRecord])(implicit ec: ExecutionContext): Future[Int]

  def updateValues(records: List[WeeklyRecords])(implicit ec: ExecutionContext): Future[Int] =
    for {
