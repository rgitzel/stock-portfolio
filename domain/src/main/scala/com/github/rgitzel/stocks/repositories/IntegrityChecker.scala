...
object IntegrityChecker {
  def checkForMissingPrices(
      accounts: List[Account],
      closingPrices: List[ClosingPrice]
  ): List[MissingPriceError] = {
    val stocksWithPricesForAGivenCurrency = closingPrices
      .map(_.currency)
      .distinct
      .map { currency =>
    ...
    val allErrors = accounts.flatMap { account =>
      account.holdingsForCurrencies.toList.flatMap {
        case (currency, holdings) =>
          val stocksHeldInThisCurrency = holdings.stocks.keys.toList
          val stocksWithoutPricesInThisCurrency =
            stocksWithPricesForAGivenCurrency.get(currency) match {
