...
object UpdatePortfolioValuesApp extends App {
  ...
  def useInfluxDbClient(influxDBClient: InfluxDBClientScala)(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    transactionsPortfolioRepository.accountJournals().flatMap { accountJournals =>
      // something like 'loaded 4 portfolio journals: INVEST (USD), LIRA (CAD, USD), RSP (CAD, USD), TFSA (CAD, USD)'
      val journalsStrings = accountJournals.map{ journal =>
      ...
      weeklyValuationProcessingResults
        ...
        .flatMap{ weeklyValuations =>
          ...
          val recordsCount = weeklyRecords.map(weeklyRecord => weeklyRecord.accountStocks.size + weeklyRecord.accounts.size + 1).sum
          println(s"retrieved data for ${weeklyRecords.size} weeks (of ${weeks.size} requested)," +
            s" and extracted ${recordsCount} records to be updated")

          // and write them out
          portfolioValueRepository.updateValues(weeklyRecords)
